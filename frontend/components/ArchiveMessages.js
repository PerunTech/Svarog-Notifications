...
class ArchiveMessages extends React.Component {
  ...
  getArchivedSubjectRecipientInfo = () => {
    ...
    const url = window.server + `/message_module/MsgServices/getSentOrArchivedSubjectRecipientInfo/${svSession}/CLOSED`
    axios.get(url)
      .then((response) => {
        if (response.data) {
    ...
    return (
      <React.Fragment>
        ...
        <div className={style['context-menu-holder']}>
          <p className={style['archive-paragraph']}>Archive Message</p>
