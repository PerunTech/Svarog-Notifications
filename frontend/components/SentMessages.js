...
class SentMessages extends React.Component {
  ...
  componentDidMount() {
    this.getSentSubjectRecipientInfo()
  }
  ...
  getMessageSubject = () => {
    ...
    axios.get(url)
      .then((response) => {
        if (response.data) {
          this.generateHtmlSentSubject(response.data)
        }
      })
      .catch((error) => {
        console.error('error')
  ...
  getSentSubjectRecipientInfo = () => {
    ...
    const url = window.server + `/SvarogNotificationsServices/getSentOrArchivedSubjectRecipientInfo/${svSession}/VALID`
    axios.get(url)
      .then((response) => {
        if (response.data) {
  ...
  generateHtmlSentSubject = (subjectData) => {
